...
require "vagrant_utm/cap"

RSpec.describe VagrantPlugins::Utm::Cap do # rubocop:disable Metrics/BlockLength
  ...
  let(:state)  { double("state", id: :started) }
  ...
  describe "#snapshot_list" do
    it "returns all the snapshots" do
      ...
      allow(driver).to receive(:list_snapshots).with(machine.id)
                                               .and_return(%w[backup old])

      expect(described_class.snapshot_list(machine)).to eq(%w[backup old])
    end
